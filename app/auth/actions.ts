...
export async function syncUserAndSetCookie(
  ...
  try {
    ...
    if (!db || !auth) {
      const err = "Firebase Admin not configured. Add FIREBASE_SERVICE_ACCOUNT_JSON (or FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY) to .env.local (local) or Vercel → Settings → Environment Variables (production).";
      console.error(err);
      return { error: err };
    }
